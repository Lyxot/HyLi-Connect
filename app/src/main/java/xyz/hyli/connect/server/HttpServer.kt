...
class HttpServer: NanoHTTPD(HTTP_PORT) {
    ...
    private fun respond(session: IHTTPSession?): Response {
        Log.i(
            TAG,
            "receive: ${session?.uri}, method = ${session?.method}, header = ${session?.headers}, " +
        ...
        if (Method.GET == session?.method) {
            ...
            // check uri
            if (session.uri in ServerConfig.NON_AUTH_URL) {
                return when (session.uri) {
                    HTTP_URL_INFO -> {
                        data = "{\"api_version\":$API_VERSION," +
                                ...
                                "\"nickname\":\"${ConfigHelper.NICKNAME}\"" +
                                "}"
            ...
            } else if (session.uri in ServerConfig.AUTH_URL) {
                ...
                data =
                    "{\"uuid\":\"${ConfigHelper.uuid}\",\"ip_addr\":\"${ConfigHelper.IP_ADDRESS}\",\"nickname\":\"${ConfigHelper.NICKNAME}\"}"
                ...
                if (session.uri == HTTP_URL_CONNECT) {
                    return if (clientMap.containsKey(uuid) && (clientMap[uuid]?.ip_addr
                            ?: "") == ip_addr
                    ) {
                    ...
                    when (session.uri) {
                        ...
                        HTTP_URL_APP_LIST -> {
                            ...
                            PackageUtils.packageList.forEach() { packageName ->
                                ...
                                e["icon"] = PackageUtils.appIconMap[packageName]?.iconByteArray
                                    ?: ByteArray(0)
                                data[packageName] = e
                            }
                            getResponse(200, data, "success")
        ...
        return getResponse(404, data, "not found")
    }
    private fun <T: Any> getResponse(code: Int, data: T, msg: String, request: String = ""): Response {
        ...
        return newFixedLengthResponse(status,
            ...
            if ( request == "" )
                ...
                "{\"code\":$code,\"data\":$data,\"msg\":\"$msg\",\"request\":\"$request\"}"
        )
    }
}