...
open class HyliConnect : Application() {
    ...
    private val onRequestPermissionResultListener =
        Shizuku.OnRequestPermissionResultListener { requestCode, grantResult ->
            if (requestCode == SHIZUKU_CODE && grantResult == PackageManager.PERMISSION_GRANTED) {
                try {
                    if (PreferencesDataStore.working_mode.getBlocking() == 1) {
                        initShizuku()
                    }
                } catch (_: Exception) { }
            }
    ...
    companion object {
        ...
        // ip: blockingQueue
        lateinit var sendMessageQueueMap: ConcurrentHashMap<String, BlockingQueue<MessageQueue>>

        // ip: set<listener>
        lateinit var receiveMessageListenerMap: ConcurrentHashMap<String, MutableSet<MessageReceiveListener>>
    ...
    override fun onCreate() {
        ...
        sendMessageQueueMap = ConcurrentHashMap()
        receiveMessageListenerMap = ConcurrentHashMap()
        me = this
        super.onCreate()
